...
if ($already) {
  "port $port already listening (pid $($already[0].OwningProcess)) at $(Get-Date -Format o)" | Out-File -FilePath $log -Append
  exit 0
}

$node   = "D:\Program Files\nodejs\node.exe"
$dshBin = Join-Path $env:LOCALAPPDATA 'npm-cache\_npx\1e7f6d9597241db0\node_modules\@deepseek-ai\dsh\lib\bin.js'
if (-not (Test-Path -LiteralPath $node))  { "node missing: $node" | Out-File -FilePath $log -Append; exit 1 }
if (-not (Test-Path -LiteralPath $dshBin)) { "dsh bin missing: $dshBin" | Out-File -FilePath $log -Append; exit 1 }
...
"started dsh pid $($p.ProcessId) at $(Get-Date -Format o)" | Out-File -FilePath $log -Append
...
if ($ok) {
  ...
  "health: port $port LISTENING (pid $pid2) at $(Get-Date -Format o)" | Out-File -FilePath $log -Append
  exit 0
}
"health: NOT listening within 90s at $(Get-Date -Format o)" | Out-File -FilePath $log -Append
exit 1
